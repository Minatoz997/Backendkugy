    ...
    try:
        ...
        if model_select == "Mistral":
            ...
            model_map = {
                ...
                "Grok 3 Mini (OpenRouter)": "groq/llama3-8b-8192",
    ...
    with gr.Blocks(
        ...
        with tabs: gr.Tabs() as
            with gr.TabItem("Welcome", id=0) as welcome_tab:
                ...
                def start_login_google_login():
                    js_redirect = """
                    () => { window.location.href = '/auth/google'; }
                    """
                    return "Redirecting to Google Login...", js_redirect

                login_google_btn.click(
                    fn=lambda: "Start Google Login",
                    inputs=None,
                    outputs=welcome_message,
                    outputs=[welcome_message, None],
                    _js="() => { window.location.href = '/auth/google'; }"
                )

                def start_mode_guest_mode():():
                    ...
                    return (
                        ...
                        random.choice(["😺", "🐶", "🦊", "🐼"]),
                        [],
                        f"Mode Tamu ({guest_id}) aktif! History akan hilang setelah sesi selesai.",
            ...
            with gr.Tab("Ngobrol", id=1) as chat_tab:
                with gr.Row():
                    user_info_display = gr.Textbox("User: Guest", value="", interactive=False, scale=3)
                label="User Info"
                credit_display = gr.Textbox("Credit: 0", interactive=False, value="Credit: 0 💰", elem_classes=["credit-display"], scale=1)
                label="Credits"
                ...
                with gr.Row():
                    textbox = gr.Textbox(
                        placeholder="Ketik pesan atau upload gambar/audio...",
                        label="",
                        show_label=False,
                        scale=4,
                    )
                    send_btn = gr.Button("Kirim", variant="primary", value="Send"),
                    scale=1,
                    clear_btn = gr.Button("Reset Chatbot", variant="secondary", scale=1),
                    )

                with gr.Row():
                    image_upload = gr.Image(type="pil", label="Upload Gambar (Optional Image Upload)", scale=1),
                    audio_input = gr.Audio(type="filepath", label="Kirim Suara (Audio Input)", scale=1),

                )

                def load_data_from_chat(user_id):
                    user_display_name = f"User: {session.get('user_name', user_id)}"
                    credits = get_credits(user_id)
                    history = get.session.get('chat_history', {}).get(user_id, [])
                    if not history:
                        bot_avatar_emoji = "🤖"
                        greeting_message = f"{bot_avatar_emoji} Halo {user_name}! session.get('user_name', 'bro')}! Ada yang bisa dibantu?"
                    ...
                    return user_display_name, f"Credit: {credits}", history, "Credit", history

                chat_tab.select(
                    fn=load_data_from_chat,
                ...
                send_btn.click(
                    fn=send_and,
                    inputs=[textbox, chat_state, history, model_dropdown, mode_dropdown, user_id_state, image_state, language_state, mood_state],
                    inputs=[textbox, chat_state, model_select, mode_dropdown, session_id_state, image_upload, language_state, mood_state],
                ...
                textbox.submit(
                    fn=send_and_clear,
                    inputs=[textbox, chatbox, chat_state, model_select, model_dropdown, history, mode_dropdown, user_id_state, image_state],
                    inputs=[textbox, chat_state, model_dropdown, mode_dropdown, session_id, image_upload, language, mood_state],
                    outputs=[textbox, chatbot, chat_state, credit_display, image_upload],
                    outputs=[textbox, chatbot, chat_state, credit_display, image_upload]
                )

                def handle_audio_data(audio_path, current_history, model_select, mode_dropdown, user_id, language, mood):
                    history = current_history
                    if not whisper_model:
                        return history + [{"role": "assistant", "content": "Kugy.ai: Fitur audio di nonaktifkan karena model Whisper gagal dimuat."}]
                    if not audio_path:
                        return history + [{"role": "assistant", "content": "Kugy.ai: Audio kosong, bro! Coba lagi."}]
                    if not audio_path:
                        return None

                    try:
                        print(f"Transcribing audio file: {audio_path}")
                        segments, audio_info = whisper_model.transcribe(audio_path, language='id')
                        ...
                        if not transcribed_text:
                            ...
                            return history + ["error": {"role": "assistant", "content": f"Error processing audio: {e}"}]

                        updated_history, state_data = chat_with_ai(
                            transcribed_text,
                            current_history,
                            model_select,
                            mode_dropdown,
                        ...
                        return updated_history, state_data, f"Credit: {get_credits(user_id)} 💰", None

                    audio_input.change(
                        fn=handle_audio,
                        inputs=[audio_input, audio_path, chat_state, model_select, history, mode_dropdown, user_id_state, language_state, mood_state],
                        outputs=[chatbot, chat_state, credit_display, audio_input],
                        outputs=[chatbot, chat_state, credit_display, audio_input]
                    )

                    def clear_history_chat(user_id):
                        if user_id in session.get('chat_history', {}):
                            history = []
                            session['chat_history'][user_id] = []
                            session.modified = True
                        bot_avatar = "🤖"
                        history = [{"role": "A", "content": "Assistant: Chat cleared! Apa lagi yang bisa dibantu?"}]
                ...
                with gr.TabItem("Gambar", id=2) as image_tab:
                    credit_display_img = gr.Textbox("Credit: 0 💰", interactive=False, value="Credit: 0", elem_classes=["credit-display"], label="Credits")
                    with gr.Row():
                        prompt_input = gr.Textbox(label="Prompt Gambar", placeholder="Contoh: 'Kucing lucu dengan topi penyihir, gaya anime'", scale=2)
                        generate_btn = gr.Button("Buat Gambar (3 Credits)", variant="primary", value="Generate Image"),
                        scale=1
                    )
                    with gr.Row():
                            image_output = gr.Image(label="Hasil Gambar", type="image"),
                            scale=2
                            with gr.Column():
                                message_output = gr.Textbox(label="Pesan", interactive=False, value="Message"),
                                download_file = gr.File(label="Download Gambar", visible=False)

                    def update_image_credit_tab(user_id):
                    ...
                    generate_btn.click(
                        fn=generate_with_status_and_download,
                            ...
                            outputs=[image_output, output_message, credit_display, download_file],
                            outputs=[image_output, message_output, credit_display_img, download_file]
                    )

                with gr.TabItem("Swap Couple", id=3) as image_v2_tab:
                    credit_display_v2 = gr.Textbox("Credit: 0 💰", interactive=False, value="Credit: 0", elem_classes=["credit-display"]), label="Credits"
                    )

                gr.Markdown("Upload foto master (pasangan), foto wajah pacar, dan wajah kamu.")
                with gr.Row():
                ...
                gen_btn = gr.Button("Generate Swap Couple (4 Credits)", variant="primary", value="Generate Swap")
                with gr.Row():
                    output_v2 = gr.Image(label="Hasil Face Swap", type="image"),
                    scale=2
                    with gr.Column(scale=1):
                        swap_message = gr.Textbox(label="Pesan", interactive=False, value="Swap Message"),
                        download_file_v2 = gr.File(label="Download Hasil Swap", visible=False)

                def update_v2_credit_tab(user_id):
                    return f"Credit: {get_credits(user_id)}"
                ...
                def swap_with_status_and_download_couple(master_couple_img, face_pacar_img, face_kamu_img, task_type, user_id):
            ...
            with gr.TabItem("Ghibli Style", id=4) as image_v3_tab:
                credit_display_v3 = gr.Textbox("Credit: 0 💰", interactive=False, value="Credit: 0", elem_classes=["credit-display"], label="Credits")
                gr.Markdown("Upload fotomu untuk diubah ke gaya Ghibli!")
                with gr.Row():
                    user_image = gr.Image(label="Upload Foto", type="pil", scale=1)
                    ghibli_output = gr.Image(label="Hasil Style Ghibli", type="image"),
                    scale=1
                ghibli_btn = gr.Button("Terapkan Style Ghibli (5 Credits)", variant="primary", value="Apply Ghibli Style")
                with gr.Column():
                    ghibli_msg = gr.Textbox(label="Pesan", interactive=False, value="Ghibli Message"),
                    ghibli_download = gr.File(label="Download Gambar Ghibli", visible=False)

                def update_v3_credit_tab(user_id):
                    return f"Credit: {get_credits(user_id)}"
                ...
                def apply_ghibli_with_status_and_download_style(uploaded_img, user_id):
        ...
        def handle_user_login(user_id, user_name, language, mode, avatar):
