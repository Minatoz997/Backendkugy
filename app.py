...
def chat_with_openrouter(message, history, user_id, model_select):
    ...
    try:
        ...
        if response.status_code != 200:
            return history + [(f"Kugy.ai: OpenRouter API error (status {response.status_code}): {response.text[:200]}", None)]
    ...
    with gr.Blocks(
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.Tab("Chat", id="Chat"):
                ...
                chat_tab = gr.get_component("Chat")
                chat_tab.select(
                    fn=load_chat_data,
        ...
        def set_login_state(is_login, user_id, user_name):
            badge_html = f'<span class="user-badge">👤 {user_id}</span>' if user_id else ""
            if is_login:
                return gr.update(visible=False), gr.update(visible=True), badge_html
            else:
        ...
        def check_initial_login(user_id_state, user_name_state, chat_history_state):
            ...
            if user_id:
                ...
                return user_id, user_name, chat_history_state, f"Welcome back! {streak_msg}", gr.update(selected="Chat"), gr.update(visible=False), gr.update(visible=True), badge_html
            else:
                return "", "", chat_history_state, "Please login or select guest mode.", gr.update(selected="Welcome"), gr.update(visible=True), gr.update(visible=False), ""

        login_btn.click(
            lambda: (gr.update(visible=True), gr.update(visible=False)),  # just open Google login in new tab; handled by /auth/login
            outputs=[welcome_col, success_col]
        )

        guest_btn.click(
            lambda chat_history_state: (
                f"guest_{int(time.time())}",
                chat_history_state,
                "Guest Mode aktif! Kamu mendapat 25 credit gratis.",
