...
def add_or_init_user(user_id: str, user_name: str = "User"):
    ...
    with sqlite3.connect("credits.db") as conn:
        ...
        c.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (user_id, user_name, default_credits, 0, "2025-06-10 20:16:06", int(time.time()), ""))
        conn.commit()
...
def save_chat_history(user_id: str, question: str, answer: str):
    with sqlite3.connect("credits.db") as conn:
        ...
        c.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (user_id, question, answer, "2025-06-10 20:16:06"))
        conn.commit()
...
def get_chat_history(user_id: str, limit: int = 20) -> List[Dict[str, str]]:
    with sqlite3.connect("credits.db") as conn:
        ...
        """, (user_id, limit))
        return [{"question": row[0], "answer": row[1], "created_at": row[2]} for row in c.fetchall()][::-1]
    ...
    async def get_available_services(self, country: str = "indonesia", service: str = "wa") -> Dict[str, Any]:
        async with httpx.AsyncClient() as client:
            try:
                ...
                logger.info(f"=== VIRTUSIM REQUEST ===\nURL: {url}\nParams: {params}")
                ...
                if response.status_code == 200:
                    data = response.json()
                    services = []
                    raw_services = data.get("data", []) if isinstance(data, dict) else data
                    for item in raw_services:
                        services.append({
                            "service_id": str(item.get("id", "")),
                            "name": item.get("name", "Unknown"),
                            "price": float(item.get("price", 0)),
                            "price_formatted": f"Rp {float(item.get('price', 0)):,.2f}",
                            "available_numbers": int(item.get("stock", 0)),
                            "country": item.get("country", country).capitalize(),
                            "status": "available" if int(item.get("stock", 0)) > 0 else "unavailable",
                            "duration": item.get("validity", "48 jam"),
                            "category": item.get("category", "OTP")
                        })
...
async def generate_image(req: ImageRequest):
    ...
    if not check_credits(req.user_email, 10):
        return JSONResponse({"error": "NOT_ENOUGH_CREDITS", "message": "Need 10 credits"}, status_code=402, headers=cors_headers)
    if not STABILITY_API_KEY:
        return JSONResponse({"error": "API_KEY_MISSING"}, status_code=503, headers=cors_headers)
    headers = {
    ...
    try:
        ...
        return JSONResponse({"error": "Failed to get image from Stability AI"}, status_code=500, headers=cors_headers)
    except Exception as e:
        logger.error(f"Error generating image: {str(e)}")
        return JSONResponse({"error": f"Error generating image: {str(e)}"}, status_code=500, headers=cors_headers)
