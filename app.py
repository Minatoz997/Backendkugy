...
def respond(audio_path, history, session_id, chat_history_state):
    if not whisper_model:
        history.append({"role": "assistant", "content": "Kugy.ai: Audio feature disabled due to Whisper model failure."})
        chat_history_state[session_id] = history
        return history, chat_history_state
    if not audio_path:
        history.append({"role": "assistant", "content": "Kugy.ai: Empty audio! Try again!"})
        chat_history_state[session_id] = history
        return history, chat_history_state
    try:
        ...
        if not message:
            history.append({"role": "assistant", "content": "Kugy.ai: Audio empty or not detected!"})
            chat_history_state[session_id] = history
            return history, chat_history_state
        ...
        traceback.print_exc()
        history.append({"role": "assistant", "content": f"Error processing audio: {str(e)}"})
        chat_history_state[session_id] = history
        return history, chat_history_state
...
def create_gradio_interface():
    with gr.Blocks(
        ...
        oauth_url_state = gr.State(value="")
        ...
        with gr.Tabs() as tabs:
            with gr.TabItem("Welcome", id=0) as welcome_tab:
                ...
                oauth_link_display = gr.HTML("", label="Google Login Link")
                ...
                def start_google_login():
                    try:
                        ...
                        link_html = f'<a href="{authorization_url}" class="oauth-link" target="_blank">Click here to Login with Google</a>'
                        return authorization_url, state, link_html, "Please click the link above to login with Google."
                    except Exception as e:
                        print(f"Error generating Google auth URL: {str(e)}")
                        return "", "", "", f"Error initiating Google login: {str(e)}"

                login_google_btn.click(
                    fn=start_google_login,
                    inputs=None,
                    outputs=[oauth_url_state, oauth_state, oauth_link_display, welcome_message]
                )
