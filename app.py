...
# Ambil API keys dari environment dengan cara yang sama seperti kode backup
API_KEYS = {
    "stability": os.getenv("STABILITY_API_KEY"),
    "openrouter": os.getenv("OPENROUTER_API_KEY")
}

# Validasi API keys dengan pendekatan seperti kode backup
missing_keys = [k for k, v in API_KEYS.items() if not v]
if missing_keys:
    logger.warning(f"Missing API keys: {', '.join(missing_keys)}")

# Validasi API keys dengan test request seperti kode backup
def validate_api_keys():
    ...
    if API_KEYS["stability"]:
        ...
        payload = {"text_prompts": [{"text": "test"}], "cfg_scale": 7, "height": 512, "width": 512, "samples": 1, "steps": 1}
        try:
    ...
    if API_KEYS["openrouter"]:
        ...
        payload = {"model": "anthropic/claude-3-haiku-20240307", "messages": [{"role": "user", "content": "test"}], "temperature": 0.7}
        try:
...
def generate_image(prompt, user_id):
    ...
    # Validasi API key sebelum digunakan seperti di kode backup
            ...
            with gr.Tab("Debug", id="Debug"):
                ...
                def get_debug_info():
                    debug_info = (
                        ...
                        f"Environment Variables: {os.environ.get('STABILITY_API_KEY', 'Not found')} (Stability), "
                        f"{os.environ.get('OPENROUTER_API_KEY', 'Not found')} (OpenRouter)"
                    )
                    return debug_info
