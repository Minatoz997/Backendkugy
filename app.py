...
app = Flask(__name__)
app.secret_key = os.getenv("FLASK_SECRET_KEY")  # Must be set in Render environment variables
if not app.secret_key:
    raise ValueError("FLASK_SECRET_KEY environment variable is not set. Please set it in Render.")
...
REDIRECT_URI = os.getenv("GOOGLE_REDIRECT_URI", "https://backend-cb98.onrender.com/auth/callback")  # Set in Render
...
def google_callback():
    try:
        ...
        print(f"Callback received. State: {state}, Request State: {request.args.get('state')}")
        if not state or state != request.args.get("state"):
            print("Invalid state parameter detected.")
        ...
        with gr.Tabs() as tabs:
            with gr.TabItem("Welcome", id=0) as welcome_tab:
                ...
                login_google_btn.click(
                    ...
                    _js="function(url) { if (url[0]) window.location.href = url[0]; }"
                )
            ...
            with gr.TabItem("Chat", id=1) as chat_tab:
                ...
                def load_chat_data(user_id):
                    ...
                    if 'chat_history' not in session:
                        session['chat_history'] = {}
                    history = session['chat_history'].get(user_id, [])
                    if not history:
                        bot_avatar_emoji = "🤖"
...
@app.route('/')
def index():
    return gradio_app.launch(server_name="0.0.0.0", server_port=port, inline=False, share=False, show_error=True)
