...
def handle_google_callback(code, state):
    try:
        ...
        if user_info_resp.status_code != 200:
            return None, None, f"Error: Gagal mengambil data Google: {user_info_resp.text}", gr.update(selected="Welcome"), True, False, ""
        ...
        with gr.Tabs() as tabs:
            with gr.Tab("Welcome", id="Welcome") as welcome_tab:
                welcome_html = gr.HTML(
                    """
                    <h3>Welcome to Kugy.ai!</h3>
                    <p>Login dengan Google untuk simpan history & dapat bonus harian, atau coba Guest Mode (history sementara).</p>
                    <div style='margin-bottom: 1em'>
                        <a class="oauth-link" href="/auth/login">Login with Google</a>
                        <button id="guest-btn" style="margin-left:5px;padding: 10px 20px; border-radius: 5px; border: none; background: #eee; color: #222; font-weight: bold;">👤 Guest Mode</button>
                    </div>
                    """, visible=True)
                ...
                success_html = gr.HTML("<div style='font-weight:bold;color:green;font-size:18px'>Login berhasil! Kamu sudah bisa chat.</div>", visible=False)
                ...
                def start_guest_mode(chat_history_state):
                    try:
                        ...
                        conn.commit()
                        conn.close()
                        return guest_id, chat_history_state, f"Guest Mode ({guest_id}) active! You have 25 free credits.", gr.update(selected="Chat"), False, "", False, ""
                    except Exception as e:
                        logger.error(f"Error in start_guest_mode: {str(e)}")
                        return None, chat_history_state, f"Error starting guest mode: {str(e)}", gr.update(selected="Welcome"), True, "", False, ""

                guest_button = gr.Button(visible=False) # Placeholder, not visible, handled by HTML/JS
                guest_button.click(
                    fn=start_guest_mode,
                    inputs=[chat_state],
                    outputs=[user_id_state, chat_state, welcome_message, tabs, welcome_html, success_html, user_badge]
                )

                callback_btn.click(
                    fn=handle_google_callback,
                    inputs=[code_input, state_input],
                    outputs=[user_id_state, user_name_state, welcome_message, tabs, welcome_html, success_html, user_badge]
                )
        ...
        def check_initial_login(user_id_state, user_name_state, chat_history_state):
            try:
                ...
                if user_id:
                    ...
                    return user_id, user_name, chat_history_state, f"Welcome back! {streak_msg}", gr.update(selected="Chat"), False, True, badge_html
                else:
                    return "", "", chat_history_state, "Please login or select guest mode.", gr.update(selected="Welcome"), True, False, ""
            except Exception as e:
                logger.error(f"Error in check_initial_login: {str(e)}")
                return "", "", chat_history_state, f"Error checking login: {str(e)}", gr.update(selected="Welcome"), True, False, ""

        demo.load(
            fn=check_initial_login,
            inputs=[user_id_state, user_name_state, chat_state],
            outputs=[user_id_state, user_name_state, chat_state, welcome_message, tabs, welcome_html, success_html, user_badge]
        )
    return demo
