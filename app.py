...
@app.get("/api/virtusim/services")
async def get_virtusim_services(country: str = Query("indonesia"), service: str = Query("wa")):
    """Get available VirtuSim services."""
    response = await virtusim_service.get_available_services(country, service)
    return JSONResponse(content=response)

@app.get("/api/virtusim/active_orders")
async def get_virtusim_active_orders():
    """Get active VirtuSim orders."""
    response = await virtusim_service.get_active_orders()
    return JSONResponse(content=response)

@app.post("/api/virtusim/order")
async def create_virtusim_order(service_id: str, operator: str = "any"):
    """Create a new VirtuSim order."""
    response = await virtusim_service.create_order(service_id, operator)
    return JSONResponse(content=response)
