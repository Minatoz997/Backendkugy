...
def handle_google_callback(code, state):
    if not code or not state:
        return None, None, "Error: Missing code or state in callback.", 0
    if state != "xyz":
        return None, None, "Error: Invalid state parameter. Possible CSRF attack.", 0
    try:
        ...
        return user_id, user_name, "Login successful! Redirecting to Chat...", 1
    except Exception as e:
        print(f"Error in Google callback: {str(e)}")
        traceback.print_exc()
        return None, None, f"Error during login: {str(e)}", 0

def extract_oauth_params_from_url(request_url):
    if not request_url:
        return None, None
    ...
    with gr.Blocks(
        ...
        with gr.Tabs() as tabs:
            with gr.TabItem("Welcome", id=0) as welcome_tab:
                ...
                # Komponen untuk menangani callback OAuth
                with gr.Row():
                ...
                # Fungsi untuk memproses callback tanpa JavaScript
                def process_callback(request_url, user_id_state, user_name_state):
                    code, state = extract_oauth_params_from_url(request_url)
                    if code and state:
                        user_id, user_name, message, tab_id = handle_google_callback(code, state)
                        return user_id, user_name, message, gr.update(selected=tab_id)
                    return user_id_state, user_name_state, "No OAuth parameters found.", gr.update(selected=0)

                # Panggil process_callback saat halaman dimuat
                demo.load(
                    fn=process_callback,
                    inputs=[gr.Request(), user_id_state, user_name_state],
