...
# Endpoint: Callback (Google OAuth2)
    ...
    with gr.Blocks(
        ...
        with gr.Tabs() as tabs:
            with gr.Tab("Welcome", id="Welcome") as welcome_tab:
                ...
                gr.Markdown("Login with Google to save history & get daily bonuses, atau coba Guest Mode (history sementara).")
                with gr.Row():
        ...
        def check_initial_login(user_id_state, user_name_state, chat_history_state):
            try:
                # FIX: biar tidak error kalau bukan State object
                ...
                logger.info(f"Checking initial login - user_id_state: {user_id}, user_name_state: {user_name}, chat_state: {chat_history_state}")
                if user_id:
