...
def top_up_credits(user_id, amount):
    ...
    c.execute("INSERT OR IGNORE INTO users (user_id, user_name, credits, login_streak, last_login, last_guest_timestamp, last_reward_date) VALUES (?, ?, ?, ?, ?, ?, ?)",
              (user_id, gr.session_state.get('user_name', user_id), 0, 0, datetime.now().strftime("%Y-%m-%d"), 0, ''))
...
def chat_with_AI(message, history, model_select, mode_dropdown, session_id, image=None, language="Indonesia", mood="Biasa"):
    ...
    if 'chat_history' not in gr.session_state:
        gr.session_state.chat_history = {}
    if user_id not in gr.session_state.chat_history:
        gr.session_state.chat_history[user_id] = []
    current_history = gr.session_state.chat_history[user_id]
    ...
    user_name = gr.session_state.get('user_name', user_id.split('@')[0] if '@' in user_id else 'bro')
    mood_messages = {
        "Pacar": {
    ...
    gr.session_state.chat_history[user_id] = current_history
...
def respond(audio_path, history, session_id):
    ...
    try:
        ...
        if 'chat_history' not in gr.session_state:
            gr.session_state.chat_history = {}
        if session_id not in gr.session_state.chat_history:
            gr.session_state.chat_history[session_id] = []
        user_avatar_emoji = assign_emoji(session_id)
        gr.session_state.chat_history[session_id].append({"role": "user", "content": f"{user_avatar_emoji} {message}"})
        return gr.session_state.chat_history[session_id]
    except Exception as e:
...
def handle_google_callback(code, state):
    if not code or not state:
        return None, None, "Error: Missing code or state in callback."
    if state != gr.session_state.get('oauth_state'):
        return None, None, "Error: Invalid state parameter. Possible CSRF attack."
    try:
        ...
        gr.session_state.user_email = user_id
        gr.session_state.user_name = user_name
        print(f"User logged in: {user_id}")
        return user_id, user_name, "Login successful! Redirecting to Chat..."
    except Exception as e:
        print(f"Error in Google callback: {str(e)}")
        traceback.print_exc()
        return None, None, f"Error during login: {str(e)}"
...
def create_gradio_interface():
    with gr.Blocks(
        ...
        with gr.Tabs() as tabs:
            with gr.TabItem("Welcome", id=0) as welcome_tab:
                ...
                def start_google_login():
                    ...
                    gr.session_state.oauth_state = state
                    print(f"Generated Google auth URL: {authorization_url}")
                    return authorization_url, "Initiating Google Login... (Redirecting)"

                login_google_btn.click(
                    fn=start_google_login,
                    inputs=None,
                    outputs=[gr.State(), welcome_message],
                    js="function(url) { if (url[0]) window.location.href = url[0]; }"
                )
                ...
                def start_guest_mode():
                    ...
                    if 'chat_history' not in gr.session_state:
                        gr.session_state.chat_history = {}
                    gr.session_state.chat_history[guest_id] = []
                    return (
                        guest_id,
                        "Indonesia",
                        "Biasa 😊",
                ...
                # Hidden components for OAuth callback
                with gr.Row(visible=False):
                    code_input = gr.Textbox(placeholder="OAuth Code", label="Code", visible=False)
                    state_input = gr.Textbox(placeholder="OAuth State", label="State", visible=False)
                    callback_btn = gr.Button("Process Callback", visible=False)
                ...
                code_input.change(
                    fn=lambda code, state: (code, state),
                    inputs=[code_input, state_input],
                    outputs=[code_input, state_input],
                    js=extract_oauth_params
                )

                callback_btn.click(
                    fn=handle_google_callback,
                    inputs=[code_input, state_input],
                    outputs=[user_id_state, gr.State(value="user_name"), welcome_message],
                    _js="() => { window.history.replaceState({}, document.title, '/'); }"
                )
            ...
            with gr.TabItem("Chat", id=1) as chat_tab:
                ...
                def load_chat_data(user_id):
                    if not user_id:
                        return "User: Unknown", "Credit: 0 💰", [], []
                    user_display_name = f"User: {gr.session_state.get('user_name', user_id)}"
                    credits = get_credits(user_id)
                    if 'chat_history' not in gr.session_state:
                        gr.session_state.chat_history = {}
                    history = gr.session_state.chat_history.get(user_id, [])
                    if not history:
                        bot_avatar_emoji = "🤖"
                        greeting_message = f"{bot_avatar_emoji} Hi {gr.session_state.get('user_name', 'bro')}! How can I help?"
                        history = [{"role": "assistant", "content": greeting_message}]
                        gr.session_state.chat_history[user_id] = history
                ...
                def clear_chat(user_id):
                    ...
                    if user_id in gr.session_state.get('chat_history', {}):
                        gr.session_state.chat_history[user_id] = history
        ...
        def check_initial_login():
            if 'user_email' in gr.session_state:
                user_id = gr.session_state.user_email
                user_name = gr.session_state.user_name
...
# Buat dan jalankan Gradio app
if __name__ == "__main__":
    port = int(os.getenv("PORT"))
    gradio_app = create_gradio_interface()
    gradio_app.launch(server_name="0.0.0.0", server_port=port, inline=False, share=False, show_error=True)