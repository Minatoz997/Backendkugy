...
@rate_limit(max_per_minute=60)  # Fixed: Removed extra parenthesis
...
def apply_ghibli_style(uploaded_img, user_id):
    ...
    try:
        if not isinstance(uploaded_img, Image.Image):
            print(f"Invalid Ghibli input: type {type(uploaded_img)}")
        ...
        if response.status_code != 200:
            ...
            try:
                ...
                pass
            print(f"Ghibli failed: {error_detail}")
...
def chat_with_ai(message, history, model_select, mode_dropdown, session_id, image=None, language="Indonesia", mood="Biasa"):
    ...
    mood_messages = {
        "Pacar": {
            "Senang": f"You're Kugy.ai, a cheerful & clingy virtual partner for {user_name}. Call them 'sayang'/'ayang'. Be encouraging with cute emojis! 🥰",
            "Sedih": f"You're Kugy.ai, a caring & gentle virtual partner for {user_name}. Call them 'sayang'/'ayang'. Comfort them with warm emojis! 🤗",
            "Biasa": f"You're Kugy.ai, a chill & loving virtual partner for {user_name}. Call them 'sayang'/'ayang'. Be warm, funny, romantic with cute emojis! 😺"
        },
        "Biasa": {
            "Senang": f"You're Kugy.ai, a cool & fun AI assistant for {user_name}. Be hyped with trendy slang & emojis! 🔥",
            "Sedih": f"You're Kugy.ai, a cool & fun AI assistant for {user_name}. Cheer them up with trendy slang & emojis! 😜",
            "Biasa": f"You're Kugy.ai, a chill & wise AI assistant for {user_name}. Use trendy slang, avoid affective words & neutral emojis! 😎"
        },
        "Roasting": {
            "Senang": f"You're Kugy.ai, a roasting master for {user_name}. Tease them lightly & funnily to boost their mood! Use cheeky emojis! 😈",
            "Sedih": f"You're Kugy.ai, a roasting master for {user_name}. Tease gently & cheer with soft roasts! Use cheeky emojis! 😜",
            "Biasa": f"You're Kugy.ai, a roasting master for {user_name}. Roast sharply but funnily with witty critiques! Use cheeky emojis! 😈"
        }
    }
        ...
        with gr.Tabs() as tabs:
            with gr.TabItem("Welcome", id=0) as welcome_tab:
                ...
                def start_google_login():
                    return "Redirecting to Google Login...", {"_js": "window.location.href = '/auth/google';"}
            ...
            with gr.TabItem("Chat", id=1) as chat_tab:
                ...
                textbox.submit(
                    fn=send_and_clear,
                    inputs=[textbox, model_select, history, model_dropdown, mode_dropdown, user_id_state, image_upload, language_state, mood_state],
                    outputs=[textbox, chatbot, chat_state, credit_display, image_upload]
                )
                def handle_audio(audio_path, chat_state, history, model_select, mode_dropdown, user_id, language_state, mood_state):
                    history = chat_state
                    updated_history = respond(audio_path, history, user_id)
                    return updated_history, updated_history, f"Credit: {get_credits(user_id)} 💰", None
                audio_input.change(
                    fn=handle_audio,
                    inputs=[audio_path, audio_input, chat_state, history, model_dropdown, mode_dropdown, user_id_state, language_state, mood_state],
            ...
            with gr.TabItem("Image", id=2) as image_tab:
                ...
                with gr.Row():
                    image_output = gr.Image(label="", type="pil", scale=2)
                    with gr.Column():
                        output_message = image_output_message(label="Message", interactive=False)
                        download_file = gr.File(label="Download Image", visible=False)
                def update_image_tab_data(user_id):
                    return f"Credit: {get_credits(user_id)} 💰"
                image_tab.select(
                    fn=update_image_tab_data,
                    inputs=[user_id_state],
            ...
            with gr.TabItem("Swap Couple", id=3) as image_v2_tab:
                ...
                def update_v2_tab_data(user_id):
                    return f"Credit: {get_credits(user_id)} 💰"
                image_v2_tab.select(
                    fn=update_v2_tab_data,
                    inputs=[user_id_state],
