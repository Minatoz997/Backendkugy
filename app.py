...
def chat_with_openrouter(message, history, user_id, model_select):
    try:
        logger.info(f"API_KEYS['openrouter'] at runtime: {'[Value Hidden]' if API_KEYS['openrouter'] else 'Not found'}")
        openrouter_key = os.getenv("OPENROUTER_API_KEY")
        logger.info(f"Re-read OPENROUTER_API_KEY at runtime: {'[Value Hidden]' if openrouter_key else 'Not found'}")
...
def handle_google_callback(code, state):
    try:
        ...
        if user_info_resp.status_code != 200:
            logger.error(f"Google userinfo response error: {user_info_resp.text}")
            return None, None, "Error: Gagal ambil data user dari Google.", gr.update(selected="Welcome")
        user_info = user_info_resp.json()
        user_id = user_info.get("email")
        if not user_id:
            logger.error("Email not found in Google userinfo response.")
            return None, None, "Error: Email tidak ditemukan di Google.", gr.update(selected="Welcome")
...
# Tambah global exception handler
...
def create_gradio_interface():
    try:
        with gr.Blocks(
            ...
            logger.info(f"Initial user_id_state: {getattr(user_id_state, 'value', None)}")
            logger.info(f"Initial user_name_state: {getattr(user_name_state, 'value', None)}")
            logger.info(f"Initial chat_state: {getattr(chat_state, 'value', None)}")
            ...
            with gr.Tabs() as tabs:
                with gr.Tab("Welcome", id="Welcome") as welcome_tab:
                    ...
                    with gr.Row():
                        gr.HTML('<a href="https://accounts.google.com/o/oauth2/v2/auth?client_id=385259735074-ui76jtbrq23idr9bk86gpbmpe06691nt.apps.googleusercontent.com&redirect_uri=https://backend-cb98.onrender.com/auth/callback&response_type=code&scope=openid%20email%20profile&state=xyz" class="oauth-link">🔑 Login with Google</a>')
            ...
            def check_initial_login(user_id_state, user_name_state, chat_history_state):
                try:
                    uid = getattr(user_id_state, "value", user_id_state) if user_id_state else None
                    uname = getattr(user_name_state, "value", user_name_state) if user_name_state else None
                    cstate = getattr(chat_history_state, "value", chat_history_state) if chat_history_state else {}
                    logger.info(f"Checking initial login - user_id_state: {uid}, user_name_state: {uname}, chat_state: {cstate}")
                    if uid:
                        streak_msg = check_login_streak(uid, uname)
                        return uid, uname, cstate, f"Welcome back! {streak_msg}", gr.update(selected="Chat")
                    return "", "", cstate, "Please login or select guest mode.", gr.update(selected="Welcome")
                except Exception as e:
                    logger.error(f"Error in check_initial_login: {str(e)}")
                    return "", "", {}, f"Error checking login: {str(e)}", gr.update(selected="Welcome")
