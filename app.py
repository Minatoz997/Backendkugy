...
FRONTEND_URL = os.getenv("FRONTEND_URL", "https://front-end-bpup-5fko2qm59-minatoz997s-projects.vercel.app/")  # Ganti sesuai domain frontend kamu
...
async def auth_google_callback(request: Request):
    ...
    # Kamu bisa simpan user/email di database, atau generate JWT, dsb.
    # Untuk demo redirect ke frontend dengan email user sebagai query param
    response = RedirectResponse(url=f"{FRONTEND_URL}/?email={user['email']}")
    return response
...
def check_login_streak(user_id, user_name):
    # For guest, just return info string
