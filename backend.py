...
async def auth_google_callback(request: Request):
    ...
    try:
        ...
        if not expected_state:
            logger.error(f"No state in session. Current session: {request.session}")
...
@app.post("/api/virtusim/reactive_order")
async def reactive_virtusim_order(id: str = Query(...)):
    """Reactivate VirtuSim order."""
    return await virtusim_service.reactive_order(id)

@app.get("/api/virtusim/check_order")
async def check_virtusim_order_status(id: str = Query(...)):
    """Check VirtuSim order status."""
    return await virtusim_service.check_order_status(id)

@app.post("/api/virtusim/set_status")
async def set_virtusim_order_status(id: str = Query(...), status: int = Query(...)):
    """Set VirtuSim order status."""
    return await virtusim_service.set_order_status(id, status)

@app.get("/api/virtusim/order_history")
async def get_virtusim_order_history():
    """Get VirtuSim order history."""
    return await virtusim_service.get_order_history()

@app.get("/api/virtusim/detail_order")
async def get_virtusim_order_detail(id: str = Query(...)):
    """Get VirtuSim order detail."""
    return await virtusim_service.get_order_detail(id)
...
@app.get("/api/virtusim/get_sms")
async def get_virtusim_sms(id: str = Query(...)):
    """Get SMS messages for an order."""
    return await virtusim_service.check_order_status(id)  # VirtuSim API doesn't have separate SMS endpoint
