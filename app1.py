...
VIRTUSIM_API_URL = "https://api.1rentpro.online"
...
def save_chat_history(user_id: str, question: str, answer: str):
    ...
    with sqlite3.connect("credits.db") as conn:
        ...
        c.execute(
            ...
            (user_id, question, answer, "2025-06-10 21:20:09"),
        )
        conn.commit()
                ...
                if response.status_code == 200:
                    try:
                        ...
                        if isinstance(data, dict) and "data" in data:
                            raw_services = data["data"]
                        else:
                            raw_services = data if isinstance(data, list) else []

                        for service in raw_services:
                            service_item = {
                                "service_id": str(service.get("id", "")),
                                "name": service.get("name", "Unknown Service"),
                                "description": f"Verifikasi {service.get('name', 'Unknown')} dengan nomor virtual",
                                "price": float(service.get("price", 0)),
                                "price_formatted": f"Rp {float(service.get('price', 0)):,.2f}",
                                "available_numbers": int(service.get("stock", 0)),
                                "country": service.get("country", country).capitalize(),
                                "status": "available" if int(service.get("stock", 0)) > 0 else "unavailable",
                                "duration": service.get("validity", "48 jam"),
                                "is_promo": bool(service.get("is_promo", False)),
...
async def get_virtusim_services(country: str = Query("Indonesia"), service: str = Query("Whatsapp")):
    ...
    try:
        ...
        logger.info(f"Timestamp: 2025-06-10 21:20:09")
        ...
        logger.error(f"Timestamp: 2025-06-10 21:20:09")
